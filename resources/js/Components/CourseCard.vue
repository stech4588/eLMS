<template>
    <div class="linkedin-card dark:bg-gray-800" :class="{ 'opacity-70': course.deleted_at }">
        ...
        <div class="linkedin-card-body dark:text-white">
            <template v-if="course.deleted_at">
                <div class="w-full rounded-md border border-red-100 bg-red-50 px-3 py-2 text-xs font-semibold text-red-600 dark:bg-red-900/20 dark:border-red-900/40 dark:text-red-200">
                    This course is temporarily unavailable.
                </div>
            </template>
            <template v-else>
                <div class="w-full bg-gray-200 rounded-full h-1.5 mb-4 dark:bg-gray-700">
                    <div class="bg-[#1C355E] h-1.5 rounded-full" :style="{ width: course.progress + '%' }"></div>
                </div>
                ...
                <div class="flex items-center justify-between mt-4 space-x-2">
                    ...
                    <Link :href="course.first_video_id ? route('courses.play', { course: course.id, video: course.first_video_id }) : '#'" 
                          class="course-action-btn-play dark:bg-[#1C355E] dark:text-white w-full">
                        Play Course
                    </Link>
                </div>
            </template>
