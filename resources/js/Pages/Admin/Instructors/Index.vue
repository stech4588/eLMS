<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Instructor Listing</h2>
        </template>
                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="bg-white dark:bg-[#2d2d2d] divide-y divide-gray-200 dark:divide-gray-700">
                                    <tr v-for="instructorUser in instructors" :key="instructorUser.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium capitalize">
                                            <span :class="{
                                                ' text-yellow-800': instructorUser.instructor.status === 'pending',
                                                ' text-green-800': instructorUser.instructor.status === 'approved',
                                                ' text-red-800': instructorUser.instructor.status === 'rejected',
                                            }" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                                                {{ instructorUser.instructor.status }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span v-if="instructorUser.instructor.status === 'approved'" :class="instructorUser.is_active ? ' text-green-800' : ' text-red-800'" class=" inline-flex text-xs leading-5 font-semibold rounded-full">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center space-x-4">
                                                <Link :href="route('admin.instructors.show', { user: instructorUser.id })" class="text-indigo-600 hover:text-indigo-900 cursor-pointer dark:invert"><img src="/images/view_icon.svg" alt="View" class="w-4 h-4"></Link>

                                                <button @click="deleteInstructor(instructorUser.id)" class="text-red-600 hover:text-red-900 cursor-pointer dark:invert"><img src="/images/delete_icon.svg" alt="Delete" class="w-4 h-4"></button>
